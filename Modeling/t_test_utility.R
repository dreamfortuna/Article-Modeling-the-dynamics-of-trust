...
dir_fits <- paste(base_dir,filesep,'Modeling',filesep,'Stan_fits_27-3seeds_U_t',sep="")
...
mu_powerL_prior_mu <- .6
mu_powerL_prior_sd <- 1
mu_powerL_lb <- .01
mu_powerL_ub <- 1
sd_powerL_lb <- .01
sd_powerL_ub <- 1
mu_powerG_prior_mu <- .4
mu_powerG_prior_sd <- 1
mu_powerG_lb <- .01
mu_powerG_ub <- 1
sd_powerG_lb <- .01
sd_powerG_ub <- 1
mu_averse_prior_mu <- 1
mu_averse_prior_sd <- 4
mu_averse_lb <- .01
mu_averse_ub <- 10
sd_averse_lb <- .01
sd_averse_ub <- 10
...
powerL_mu_vector <- array(0, length(subjects))
powerL_sd_vector <- array(0, length(subjects))
powerG_mu_vector <- array(0, length(subjects))
powerG_sd_vector <- array(0, length(subjects))
averse_mu_vector <- array(0, length(subjects))
averse_sd_vector <- array(0, length(subjects))
...
for (subi in 1:Nsubjects){
  ...
  setwd(dir_file)
  SIGMA <- readMat('sigma_U.mat')
  sigmas_mu_vector[subi] <- SIGMA$sigmas.mu[subi]
  sigmas_sd_vector[subi] <- SIGMA$sigmas.sd[subi]

  setwd(dir_file)
  BETA0 <- readMat('beta0_U.mat')
  beta0_mu_vector[subi] <- BETA0$beta0.mu[subi]
  beta0_sd_vector[subi] <- BETA0$beta0.sd[subi]

  setwd(dir_file)
  BETA1 <- readMat('beta1_U.mat')
  beta1_mu_vector[subi] <- BETA1$beta1.mu[subi]
  beta1_sd_vector[subi] <- BETA1$beta1.sd[subi]

  setwd(dir_file)
  ALPHA <- readMat('alpha_U.mat')
  alpha_aa_vector[subi] <- ALPHA$alpha.aa[subi]
  alpha_ab_vector[subi] <- ALPHA$alpha.ab[subi]

  setwd(dir_file)
  PPK <- readMat('ppk_U.mat')
  ppk_40_vector[subi] <- PPK$ppk.40[subi]
  ppk_80_vector[subi] <- PPK$ppk.80[subi]
  ppk_120_vector[subi] <- PPK$ppk.120[subi]

  setwd(dir_file)
  POWERL <- readMat('powerL.mat')
  powerL_mu_vector[subi] <- POWERL$powerL.mu[subi]
  powerL_sd_vector[subi] <- POWERL$powerL.sd[subi]

  setwd(dir_file)
  POWERG <- readMat('powerG.mat')
  powerG_mu_vector[subi] <- POWERG$powerG.mu[subi]
  powerG_sd_vector[subi] <- POWERG$powerG.sd[subi]

  setwd(dir_file)
  AVERSE <- readMat('averse.mat')
  averse_mu_vector[subi] <- AVERSE$averse.mu[subi]
  averse_sd_vector[subi] <- AVERSE$averse.sd[subi]
...
for (m in 1:length(models)) {
  ...
  data <- auto_name_list(Nblocks=Nblocks, maxTrials=Ntrials, Ns=Nsubjects, subNtrials=subNtrials,
               ...
               powerL_mu=powerL_mu_vector, powerL_sd=powerL_sd_vector, powerG_mu=powerG_mu_vector, powerG_sd=powerG_sd_vector,
               averse_mu=averse_mu_vector, averse_sd=averse_sd_vector,
  ...
  for (iter in 1:Nreruns) {
    ...
    seed <- sample(1:10000,1)
      parameters <- c('mu_powerL','mu_powerG','mu_averse','powerL','powerG','averse','sigma','mu_beta0','beta0','mu_beta1','beta1','mu_alpha','alpha','ppk','ppx','ppCs','ppCo','ppEVr','ppPgamble','ppd','ppu','lik','log_lik')
      group_param <- c('mu_powerL','mu_powerG','mu_averse','mu_beta0','mu_beta1','mu_alpha')
      subject_param <- c('powerL','powerG','averse','sigma','beta0','beta1','alpha')
    ...
    subject_predict <- NULL
      ...
      subject_param$powerL <- colMeans(samp$powerL)
      subject_param$powerG <- colMeans(samp$powerG)
      subject_param$averse <- colMeans(samp$averse)
      
      
      subject_param$powerL_sd <- apply(samp$powerL, 2, sd, na.rm = TRUE)
      subject_param$powerG_sd <- apply(samp$powerG, 2, sd, na.rm = TRUE)
      subject_param$averse_sd <- apply(samp$averse, 2, sd, na.rm = TRUE)
