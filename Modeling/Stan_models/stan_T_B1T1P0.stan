...
model {
  ...
  for (s in 1:Ns) {
        ...
        sigma[s] ~ normal(noise[s],0.001);
        bds[s] ~ normal(0.1,0.001);
        alpha[s] ~ beta(aa,ab);
        beta0[s] ~ normal(mu_beta0, sd_beta0);
        beta1[s] ~ normal(mu_beta1, sd_beta1);
        ...
        bd[1] = 0;
            ...
            for (i in 1:subNtrials[b,s]) {
                ...
                if (self[b,i,s] == 1) {
                  ...
                  EVr = 20*PcorCONDx_o - 20*(1-PcorCONDx_o);
				          EVs = 5*PcorCONDx_s - 5*(1-PcorCONDx_s);
                }
